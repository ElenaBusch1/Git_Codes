...
void PtRegression_to_DNN_Images()
{
	...
	ofstream output("GreyScale_Image_Test.csv");
	...
	for(Long64_t iEntry = 0; iEntry <numEvents; iEntry++){
		...
		if (DATATYPE == 1){
			cout << "Found for loop" << endl;
			output << GEN_pt << endl;
			int mat_vals[64][64] = {0};
			mat_vals[row_12][col_12] += 1;
			mat_vals[row_23][col_23] += 2;
			mat_vals[row_34][col_34] += 4;
			for (int i=0; i<64; i++){
                                for(int j=0; j<64; j++){
					if (j == 63){
                                 	output <<mat_vals[i][j]<< endl;}//last entry
					else{
					output <<mat_vals[i][j]<<",";}
                                }
			}
			output << endl; //skip line between matricies
                	TCanvas *c = new TCanvas("Color","ColorScale", 70,70);
			c->SetBorderMode(0);
			Float_t n1 = 1./64;
			Int_t n = 64;
			for (int i = 0; i<64; i++){
        			for (int j = 0; j<64; j++){
                			...
                			int col = mat_vals[i][j];
                			b->SetFillColor(col);
                			b->Draw();
 				}
			}		
		TGaxis *xax = new TGaxis(20, 20, 20, 65,-8, 47, 64,"");
		TGaxis *yax = new TGaxis(20, 20, 65, 20, 0, 1050, 64,"");
		xax->SetName("dTh");
		yax->SetName("dPhi");
		xax->Draw();
		yax->Draw();
